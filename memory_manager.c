...
{
	int newSize = ROUNDUP(size, PAGE_SIZE)/PAGE_SIZE ;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//[1] remove pages from disk
	for (int i = index ; i <(index+size); i++ )
	{
		pf_remove_env_page(e, new_va);
		new_va += PAGE_SIZE;
	}
	//[2]remove from WorkingSet.
	index =(index+size); //index of the end
	uint32 ptr= virtual_address;
	uint32 Va =((PAGE_SIZE * (index)) + USER_HEAP_START);
//	cprintf("max loop : %d \n",Va);
//	cprintf("max loop : %d \n",(virtual_address+(size* PAGE_SIZE)));
	for(int k=ptr;k<Va;k+=PAGE_SIZE)
	{
		uint32 * retpage = NULL;
		get_frame_info(e->env_page_directory, (void*)k, &retpage);
		if (retpage != NULL )
		{
			for (int i = 0; i < e->page_WS_max_size; i++)
			{
				if (ROUNDDOWN(e->ptr_pageWorkingSet[i].virtual_address,PAGE_SIZE)== k)
				{
					uint32 *ptr_page_table=NULL;
					struct Frame_Info *ptr_frame_info =get_frame_info(e->env_page_directory,(void*)k,&ptr_page_table);
					free_frame(ptr_frame_info);
					unmap_frame(e->env_page_directory,(void*)k);
			...
			{
				  if(( retpage[i]&PERM_PRESENT)==PERM_PRESENT)
				  {
					  Mycheck=1;
					  break;
				  }
			...
			if(Mycheck==0)
			{
				uint32 pa =(uint32)retpage - KERNEL_BASE;
				struct Frame_Info * FI2=to_frame_info(pa);
				free_frame(FI2);
				e->env_page_directory[PDX(k)] = 0;
			}
		}
	}
}
